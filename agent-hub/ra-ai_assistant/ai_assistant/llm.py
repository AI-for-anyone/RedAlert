...
class LLMClient(BaseNode):
    ...
    async def _get_system_prompt_async(self) -> str:
        ...
        if  unit_tool is None:
            print("未找到 unit_info_query 工具，使用默认提示词")
            return self._get_system_prompt()
        if base_tool is None:
            print("未找到 player_base_info_query 工具，使用默认提示词")
            return self._get_system_prompt()
        if produce_tool is None:
            print("未找到 query_production_queue 工具，使用默认提示词")
            return self._get_system_prompt()
        print(f"工具ready")
    ...
    async def node(self, task_input: str) -> str:
        ...
        try:
            ...
            # 使用LLM和工具执行任务
                ...
                print(f"AI 助手执行结果: {result}")
                loop_times += 1
                print(f"AI 助手循环次数: {loop_times}")
